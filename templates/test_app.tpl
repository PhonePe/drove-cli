...

{
    ...
    "healthcheck": {
        "mode": {
            ...
            "protocol": "HTTP",
            "portName": "main",
            "path": "${reader.read_str('checkPath', 'Healthcheck Path', pattern='^(?P<path>/[a-zA-Z0-9\-._~!$&\'()*+,;=:@%\/]*)(?:\?(?P<query>[a-zA-Z0-9\-._~!$&\'()*+,;=:@%\/?]*))?$', default='/', max_length=1024)}",
            "verb": "GET",
    ...
    "readiness": {
        "mode": {
            ...
            "protocol": "HTTP",
            "portName": "main",
            "path": "${reader.read_str('checkPath', 'Healthcheck Path', pattern='[a-zA-Z0-9\-/%?=]+', default="/", max_length=1024)}",
            "verb": "GET",
            "successCodes": [ 200, 201, 202 ],
            "payload": "",
            "connectionTimeout": "1 second"
