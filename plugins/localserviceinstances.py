...
class LocalServices(plugins.DrovePlugin):
    ...
    def populate_options(self, drove_client: droveclient.DroveClient, subparser: argparse.ArgumentParser):
        ...
        sub_parser = commands.add_parser("list", help="List all local service instances")
        ...
        sub_parser = commands.add_parser("info", help="Print details for an local service instance")
        sub_parser.add_argument("service_id", metavar="service-id", help="Local Service ID")
        ...
        sub_parser = commands.add_parser("logs", help="Print list of logs for local service instance")
        sub_parser.add_argument("service_id", metavar="service-id", help="Local Service ID")
        sub_parser.add_argument("instance_id", metavar="instance-id", help="Local Service Instance ID")
        sub_parser.set_defaults(func=self.show_logs_list)

        sub_parser = commands.add_parser("tail", help="Tail log for local service instance")
        sub_parser.add_argument("service_id", metavar="service-id", help="Local Service ID")
        sub_parser.add_argument("instance_id", metavar="instance-id", help="Local Service Instance ID")
        ...
        sub_parser.add_argument("service_id", metavar="service-id", help="Local Service ID")
        sub_parser.add_argument("instance_id", metavar="instance-id", help="Local Service Instance ID")
        sub_parser.add_argument("servicelogfile", help="Log filename to download")
        ...
        sub_parser = commands.add_parser("replace", help="Replace specific local service instances with fresh instances")
        sub_parser.add_argument("service_id", metavar="service-id", help="Local Service ID")
        sub_parser.add_argument("instance_ids", nargs="+", metavar="instance-id", help="Local Service Instance IDs")
        sub_parser.add_argument("--stop", "-s", action='store_true', help="Stop the instance before spinning up a new one", default = False)
        ...
        sub_parser = commands.add_parser("kill", help="Kill specific local service instances")
        sub_parser.add_argument("service_id", metavar="service-id", help="Local Service ID")
        sub_parser.add_argument("instance_ids", nargs="+", metavar="instance-id", help="Local Service Instance IDs")
        sub_parser.add_argument("--parallelism", "-p", help="Number of parallel threads to be used to execute operation", type=int, default = 1)
        sub_parser.add_argument("--timeout", "-t", help="Timeout for the operation on the cluster", type=str, default = "5m")
        ...
        # sub_parser = commands.add_parser("create", help="Create local service")
        # sub_parser.add_argument("definition", help="JSON local service definition")
        
        super().populate_options(drove_client, parser)


    def list_instances(self, options: SimpleNamespace):
        api = "/apis/v1/localservices/{service_id}/instances"
        if options.old:
    ...
    def show_instance(self, options):
        raw = self.drove_client.get("/apis/v1/localservices/{service_id}/instances/{instance_id}".format(service_id = options.service_id, instance_id=options.instance_id))
        data = dict()
        data["Instance ID"] = raw["instanceId"]
        data["Service ID"] = raw["serviceId"]
    ...
    def log_download(self, options):
        ...
        droveutils.download_log(self.drove_client, "localservices", options.service_id, options.instance_id, options.servicelogfile, filename)
    ...
    def replace(self, options):
        ...
        if options.wait:
            self.ensure_replaced(options.service_id, set(options.instance_ids))
    ...
    def ensure_replaced(self, service_id: str, existing: set) -> bool:
        healthy = self.drove_client.app_instances(service_id, False)
