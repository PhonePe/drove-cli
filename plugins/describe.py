"""
Describe plugin for drove-cli - detailed human-readable resource information.
...
class Describe(plugins.DrovePlugin):
    """Plugin for describe commands - detailed resource views."""
    ...
    def describe_app(self, options: SimpleNamespace):
        ...
        if instances:
            self._print_section(f"Instances ({len(instances)})")
            for inst in instances:
                state_marker = self._get_state_marker(inst['state'])
    ...
    def describe_cluster(self, options: SimpleNamespace):
        ...
        if executors:
            self._print_section(f"Executors ({len(executors)})")
    ...
    def _get_resource_count(self, item: dict, resource_type: str) -> int:
        """Get count of a resource type (e.g., CPU cores)."""
    ...
    def _get_resource_sum(self, item: dict, resource_type: str) -> int:
        """Get sum of a resource type (e.g., memory in MB)."""
        resources = item.get('resources', [])
        for res in resources:
            if res.get('type') == resource_type:
